...
export const ActSearchResultItem = ({ item }: { item: Act }) => {
  ...
  const meetingDate = item.meetingDate ? format(parseISO(item.meetingDate), "PPP", { locale: es }) : "Sin fecha";

  return (
    <Link to={`/books/${item.volumeId}`} state={{ initialActId: item.id }} className="block p-4 border rounded-lg hover:bg-muted/50 transition-colors">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-muted-foreground">
              En: {item.volumeName} ({item.bookName}) • Mod. por {modifier} • {timeAgo}
            </p>
          </div>
        </div>
        <Badge variant="outline" className="capitalize">{item.status.toLowerCase()}</Badge>
      </div>
