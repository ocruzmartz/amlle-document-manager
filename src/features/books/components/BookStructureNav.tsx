...
export const BookStructureNav = ({
  ...
  const handleActClick = (actId: string) => {
    onViewChange({
      main: { type: "acta-edit", actaId: actId },
  ...
  const handleAgreementClick = (actId: string, agreementId: string) => {
    onViewChange({
      main: { type: "acta-edit", actaId: actId }, // Mantiene el acta en la columna principal
      detail: { type: "agreement-editor", agreementId: agreementId }, // Abre el editor del acuerdo en la 3ra columna
      activeActId: actId,
    });
  };
